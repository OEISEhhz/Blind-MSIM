...
%                           a weighted matrix.
%           opts.max_iter: maximum iterations of the algorithm. 算法最大迭代次数
%                           Default 300.
...
%     opts = initOpts(opts);
    ...
    while  iter < opts.max_iter
        iter = iter + 1;
        x_new = feval(proj, y_old - Linv*feval(grad, y_old), opts_proj); %X = proj_l1(U, opts)
        t_new = 0.5*(1 + sqrt(1 + 4*t_old^2));
        y_new = x_new + (t_old - 1)/t_new * (x_new - x_old);
        %% check stop criteria
        e = norm1(x_new - x_old)/numel(x_new);%norm1(X) = full(sum(abs(X(:))));    numel：求x_new中的元素数
        if e < opts.tol
            break;
        end
        %% update
